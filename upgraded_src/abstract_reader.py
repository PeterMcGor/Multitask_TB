...
class Data_Workaround(object):
    ...
    def data_augmenting(self, image, mask=None):
        ...
        images_to_flip = [img, mask] if isinstance(mask, np.ndarray) else [img]
    ...
    def data_workaround(self, img_path):
        ...
        # print("IMG_MIN", np.min(img), "IMG_MAX", np.max(img), "IMG_SHAPE", img.shape)
        meta_info = 'None'
        name = str(os.path.basename(img_path))
        mask = 'None'  # Tensorflow and proper none types don't like each other
        y = 'None'
        if self.aim != Aim.CLASSIFICATION:
            ...
            # print("MASK_MIN",np.min(mask), "MASK_MAX",np.max(mask),
            # "MASK_SHAPE", mask.shape, 'ZEROS',np.sum(mask == 0), 'ones', np.sum(mask))
        ...
        if self.mode == tf.estimator.ModeKeys.TRAIN:
            ...
            # print('PAAAAATH', path)
            # img_to = img.transpose()
            # mask_to = mask.transpose()
            # img_to = sitk.GetImageFromArray(img_to)
            # mask_to = sitk.GetImageFromArray(mask_to)
            # sitk.WriteImage(img_to, os.path.join(path, name+'.nii'))
            # sitk.WriteImage(mask_to, os.path.join(path, name + '_mask.nii'))
        img = img[:, :, :, np.newaxis]
...
class Reader(object):
    ...
    def get_inputs(self,
        ...
        def train_inputs():
            ...
            print('LABEL Types', labels_types)
            print('LABEL Shapes', labels_shape)
            print('TYPES', self.dtypes)

            dataset = dataset.map(lambda filename: tuple(tf.compat.v1.py_func(data_workd.data_workaround, [filename],
            ...
            dataset = dataset.prefetch(2)  # TODO OJO que esto puede reventar
            return dataset
    ...
    def serving_input_receiver_fn(self, placeholder_shapes):
        ...
        def f():
            inputs = {k: tf.compat.v1.placeholder(
