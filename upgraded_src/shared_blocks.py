...
def fully_connected_block(x, units=[4096, 2048, 1024, 256], activation=tf.nn.relu6,
    ...
    with tf.compat.v1.variable_scope(scope_name):
        ...
        for i, unit in enumerate(units):
            ...
            x = tf.compat.v1.layers.dense(x, units=unit, name=name, activation=activation,
                                kernel_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))
            x = dropout_selu(x, rate=selu_rates[i], training=training) if activation == tf.nn.selu else x

            x_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES,
                                       scope=tf.compat.v1.get_default_graph().get_name_scope() + '/' + name)

            if len(x_vars) > 1:
                tf.compat.v1.summary.histogram(name + "_weights", x_vars[0])
                tf.compat.v1.summary.scalar(name + "_mean_weights", tf.reduce_mean(input_tensor=x_vars[0]))
                tf.compat.v1.summary.histogram(name + "_bias", x_vars[1])
        return x
...
def cnn_shared_features(x, activation=tf.nn.relu6, dropout_rate=[0.1, 0.1, 0.1, 0.1],
    ...
    for i, n_filters in enumerate(num_of_filters):
        for convolution in range(num_of_convolutions[i]):
            x = tf.compat.v1.layers.conv3d(x, n_filters, 1, padding="same")
            x = tf.compat.v1.layers.batch_normalization(x, training=training, renorm=True)
            x = activation(x)
            x = tf.compat.v1.layers.dropout(x, rate=dropout_rate[i], training=training)
...
def specific_cnn_by_manifestation(x, manifestations=None, activation=tf.nn.relu6, input_size=[8, 8, 8], mode=None):
    ...
    for manifestation in manifestations:
        manifestation_x = x
        with tf.compat.v1.variable_scope('down_convolution'):
            for l in range(levels):
                with tf.compat.v1.variable_scope(manifestation + '/encoder/level_' + str(l + 1)):
                    ...
                    manifestation_x = tf.compat.v1.layers.conv3d(manifestation_x, shape_x[-1], kernel_size=kernel_s, strides=2)
                    manifestation_x = tf.compat.v1.layers.batch_normalization(manifestation_x, training=training, renorm=True)
                    manifestation_x = activation(manifestation_x)
                    print('L', l, manifestation + '/encoder/level_' + str(l + 1), manifestation_x)
        manifestations_features[manifestation] = tf.compat.v1.layers.flatten(manifestation_x)
...
def vnet_decoder_style(x, v_net_model=None, manifestations=None, task_up_levels=None, mode=None,
    ...
    for manifestation in manifestations:
        ...
        with tf.compat.v1.variable_scope(manifestation + '_features'):
            ...
            manifestation_x = tf.compat.v1.layers.conv3d(manifestation_x, 128, [8, 8, 8], strides=[8, 8, 8])
            manifestation_x = tf.compat.v1.layers.batch_normalization(manifestation_x, training=is_training, renorm=True)
            manifestation_x = activation(manifestation_x)
            print(manifestation + 'get_features_middle', manifestation_x)

            manifestation_x = tf.compat.v1.layers.conv3d(manifestation_x, 256, [4, 4, 4], strides=1)
            manifestation_x = tf.compat.v1.layers.batch_normalization(manifestation_x, training=is_training, renorm=True)
            manifestation_x = activation(manifestation_x)
            manifestations_features[manifestation] = tf.compat.v1.layers.flatten(manifestation_x)
