...
def kendall_gal_loss_clf_sm_fl(labels, mask, clf_logits, seg_logits, log_vars_clf, log_var_seg, eps=1e-7):
    ...
    return tf.reduce_mean(input_tensor=loss), loss_acc
...
def kendall_gal_loss_sm_fl(ys_true, ys_pred, ys_tasks_type, log_vars=None, gamma=2):
    def specific_task_loss(y_true, y_pred, log_var_loss, name, task_type):
        ...
        if task_type is TASK_TYPE.BINARY:
            ...
            y_pred = tf.nn.softmax(y_pred * precision, axis=-1)
            L = tf.reduce_sum(input_tensor=-y_true_hot * ((1 - y_pred) ** gamma) * tf.math.log(y_pred),
                              axis=1, keepdims=True, name='TASK_LOSS_' + name)
            # L should be (bs,1)
            return L
        else:
            return tf.add(precision * tf.pow(y_true - y_pred, 2) / 2., log_var_loss / 2, name='TASK_LOSS_' + name)

    shape = tf.shape(input=ys_true[IMAGE_CLEF_KEYS.CTR_CAVERNS])
    ...
    for i, log_var_name in enumerate(log_vars):
        ...
        mean_per_task_loss = tf.reduce_mean(input_tensor=task_loss, name='TASK_LOSS_Mean' + log_var_name)
        tf.compat.v1.summary.scalar('loss_' + log_var_name, mean_per_task_loss)
...
def uncertainty_loss_sm_fl_c(labels, mask, clf_logits, seg_logits, log_vars_clf, log_vars_seg):
    ...
    return tf.reduce_mean(input_tensor=result), loss_acc
...
def sigmoid_cross_entropy_seg_unc(labels, mask, clf_logits, seg_logits, log_vars_clf, log_vars_seg):
    loss = tf.compat.v1.losses.sigmoid_cross_entropy(mask, seg_logits, reduction=tf.compat.v1.losses.Reduction.NONE)

    precision = tf.exp(-log_vars_seg)
    task_loss = tf.multiply(precision, loss) + log_vars_seg / 2.
    tf.compat.v1.summary.histogram('uncertainty_seg', task_loss)

    loss = tf.reduce_mean(input_tensor=task_loss)
    return loss, loss
...
def sigmoid_cross_entropy_seg(labels, mask, clf_logits, seg_logits, log_vars_clf, log_vars_seg):
    ...
    loss = tf.compat.v1.losses.sigmoid_cross_entropy(mask, seg_logits)
...
def sigmoid_cross_entropy_per_sample(labels, mask, clf_logits, seg_logits, log_vars_clf, log_vars_seg):
    loss = tf.compat.v1.losses.sigmoid_cross_entropy(mask, seg_logits, reduction=tf.compat.v1.losses.Reduction.NONE)
    loss = tf.reduce_mean(input_tensor=loss, axis=[1, 2, 3], name='CE_per_sample')
    print('sigmoid cross entropy loss', loss)
    return loss, loss
...
def kendall_gal_loss_clf2(labels, mask, clf_logits, seg_logits, log_vars_clf, log_var_seg, eps=1e-7):
    ...
    for i, log_var_name in enumerate(log_vars_clf):  # TODO REVISAR ####################################
        ...
        wo_loss = tf.abs(labels[log_var_name] - clf_logits[log_var_name]) if task_types[
            ...
            tf.compat.v1.losses.sigmoid_cross_entropy(labels[log_var_name], clf_logits[log_var_name])
        spec_task_loss = (precision / 2.) * tf.abs(labels[log_var_name] - clf_logits[log_var_name]) + (log_var / 2.) \
            ...
            tf.compat.v1.losses.sigmoid_cross_entropy(labels[log_var_name], clf_logits[log_var_name] * precision)
        tf.compat.v1.summary.scalar('wo/var_loss_' + log_var_name, tf.reduce_mean(input_tensor=wo_loss))

        task_loss = spec_task_loss

        mean_per_task_loss = tf.reduce_mean(input_tensor=task_loss, name='TASK_LOSS_' + log_var_name)
        tf.compat.v1.summary.scalar('loss_' + str(log_var_name), mean_per_task_loss)  # Show in tensorboard
        # mean_task_loss = tf.reduce_mean(task_loss) #Probatura
        loss = tf.add(loss, task_loss, name='LOSS_ADD_' + log_var_name)
        loss_acc.append(mean_per_task_loss)
    return tf.reduce_mean(input_tensor=loss, name='REDUCED_LOSS'), loss_acc
...
def kendall_gal_loss_sm(ys_true, ys_pred, ys_tasks_type, log_vars=None, eps=1e-7):
    ...
    def specific_task_loss(y_true, y_pred, task_type):
        ...
        if task_type is TASK_TYPE.BINARY:
            ...
            return tf.compat.v1.losses.softmax_cross_entropy(y_true_hot, y_pred, reduction=tf.compat.v1.losses.Reduction.NONE)
        else:
    ...
    for i, log_var_name in enumerate(log_vars):
        ...
        tf.compat.v1.summary.scalar('wo/var_loss_' + log_var_name, tf.reduce_mean(input_tensor=spec_task_loss))
        task_loss = tf.multiply(precision, spec_task_loss, name='TASK_LOSS_' + log_var_name) + log_var / 2.

        mean_per_task_loss = tf.reduce_mean(input_tensor=task_loss, name='TASK_LOSS_Mean' + log_var_name)
        tf.compat.v1.summary.scalar('loss_' + log_var_name, mean_per_task_loss)  # Show in tensorboard
        # mean_task_loss = tf.reduce_mean(task_loss) #Probatura
        loss = tf.add(loss, task_loss, name='LOSS_ADD_' + log_var_name)
        loss_acc[log_var_name] = mean_per_task_loss
    return tf.reduce_mean(input_tensor=loss, name='REDUCED_LOSS'), loss_acc
...
def kendall_gal_loss(ys_true, ys_pred, ys_tasks_type, log_vars=None, eps=1e-7):
    ...
    def specific_task_loss(y_true, y_pred, task_type):
        y_true = tf.cast(y_true, tf.float32)
        if task_type is TASK_TYPE.BINARY:
            return tf.compat.v1.losses.sigmoid_cross_entropy(y_true, y_pred, reduction=tf.compat.v1.losses.Reduction.NONE) + eps
        else:
