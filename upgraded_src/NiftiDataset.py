...
class NiftiDataset(object):
  ...
  def get_dataset(self):
    ...
    dataset = dataset.map(lambda image_path, label_path: tuple(tf.compat.v1.py_func(
  ...
  def get_classification_dataset(self):
    ...
    for case in os.listdir(self.data_dir):
      image_paths.append(os.path.join(self.data_dir,case,self.image_filename))
      #label_paths.append(os.path.join(self.data_dir,case,self.label_filename))
    label_paths = [self.label_filename]*len(image_paths)

    dataset = tf.data.Dataset.from_tensor_slices((image_paths,label_paths))

    dataset = dataset.map(lambda image_path, label_path: tuple(tf.compat.v1.py_func(
      self.input_classification_parser, [image_path, label_path], [tf.float32,tf.int32])))
  ...
  def input_classification_parser(self,image_path, label_path):
    def csv_parser():
        with file_io.FileIO(label_path, mode ='r') as f:
            annotatations_pandas = pd.read_csv(f)
        
        name = image_path.split(os.sep)[-1]+'.gz'
