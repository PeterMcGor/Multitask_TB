"""
Created on Tue Mar 22 12:48:22 2019

VNet Layers  based on Miguel Monteiro's
...
def xavier_initializer_convolution(shape, dist='uniform', lambda_initializer=True):
    ...
    if dist == 'uniform':
        ...
        if lambda_initializer:
            ...
            return tf.random.uniform(shape, minval=-lim, maxval=lim)
    if dist == 'normal':
        stddev = np.sqrt(3. / num_activations)
        if lambda_initializer:
            return np.random.normal(0, stddev, shape).astype(np.float32)
        else:
            tf.random.truncated_normal(shape, mean=0, stddev=stddev)
...
def prelu(x):
    alpha = tf.compat.v1.get_variable('alpha', shape=x.get_shape()[-1], dtype=x.dtype, initializer=tf.compat.v1.constant_initializer(0.1))
...
def convolution(x, filter, padding='SAME', strides=None, dilation_rate=None):
    ...
    tf.cond(pred=tf.equal(tf.keras.backend.any(tf.math.is_nan(w)), False), true_fn=lambda: tf.compat.v1.summary.histogram(scope_name+'/weights', w),
            false_fn=lambda: tf.as_string(w))
    tf.cond(pred=tf.equal(tf.keras.backend.any(tf.math.is_nan(b)), False), true_fn=lambda: tf.compat.v1.summary.histogram(scope_name + '/biases', b),
            false_fn=lambda: tf.as_string(b))

    # tf.summary.histogram(scope_name + '/biases', b)

    return tf.nn.convolution(input=x, filters=w, padding=padding, strides=strides, dilations=dilation_rate) + b
...
def deconvolution(x, filter, output_shape, strides, padding='SAME'):
    ...
    scope_name = tf.compat.v1.get_default_graph().get_name_scope()
    tf.compat.v1.summary.histogram(scope_name + '/weights', w)
    tf.compat.v1.summary.histogram(scope_name + '/biases', b)
