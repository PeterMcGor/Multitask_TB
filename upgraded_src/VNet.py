...
top_scope = tf.compat.v1.get_variable_scope()
...
def convolution_block(layer_input, num_convolutions, keep_prob, activation_fn, is_training=True, kernel_size=[5, 5, 5],
    ...
    for i in range(num_convolutions):
        with tf.compat.v1.variable_scope('conv_' + str(i+1)):
            ...
            x = tf.compat.v1.layers.batch_normalization(x, training=is_training, renorm=renorm) if batch_normalization else x
            # check shit
            print("GRAPH_NAME", tf.compat.v1.get_default_graph().get_name_scope())  # SUS MUERTOS
            tf.reduce_mean(input_tensor=x, name='MEAN_BN')
            tf.reduce_min(input_tensor=x, name='MIN_BN')
            tf.reduce_max(input_tensor=x, name='MAX_BN')

            if i == num_convolutions - 1:
                x = x + layer_input
                x = activation_fn(x) if activation_after_add else x
                x = tf.nn.dropout(x, 1 - (keep_prob))
                return x
...
def convolution_block_2(layer_input, fine_grained_features, num_convolutions, keep_prob, activation_fn, is_training=True,
    ...
    if num_convolutions == 1:
        with tf.compat.v1.variable_scope('conv_' + str(1)):
    ...
    for i in range(1, num_convolutions):
        with tf.compat.v1.variable_scope('conv_' + str(i+1)):
            ...
            x = tf.compat.v1.layers.batch_normalization(x, training=is_training, renorm=False) if batch_normalization else x
            if i == num_convolutions - 1:
                x = x + layer_input
                x = activation_fn(x) if activation_after_add else x
                x = tf.nn.dropout(x, 1 - (keep_prob))
...
#
        ...
        with tf.compat.v1.variable_scope('vnet/output_layer'):
            logits = convolution(x, [1, 1, 1, self.num_channels, self.num_classes])
        with tf.compat.v1.variable_scope('vnet/output_layer/log_var'):
    ...
    def encoder_fn(self, x, mode):
        ...
        with tf.compat.v1.variable_scope('vnet/input_layer'):
            if input_channels == 1:
                x = tf.tile(x, [1, 1, 1, 1, self.num_channels])
                x = tf.compat.v1.layers.batch_normalization(x, training=is_training, renorm=self.renorm) \
        ...
        for l in range(self.num_levels):
            ...
            with tf.compat.v1.variable_scope('vnet/encoder/level_' + str(l + 1)):
        ...
        with tf.compat.v1.variable_scope('vnet/bottom_level'):
    ...
    def decoder_task(self, mode, task_name, task_up_levels=None):
        ...
        for l in [level for level in reversed(range(self.num_levels))][:task_up_levels]:
            print('Level up', l)
            with tf.compat.v1.variable_scope('vnet/decoder/'+task_name+'/level_' + str(l)):
                f = self.level_features['level_' + str(l)]
                with tf.compat.v1.variable_scope('up_convolution'):
                    print('vnet/decoder/level_' + str(l + 1), task_name, '_input', x)
                    x = up_convolution(x, tf.shape(input=f), factor=2, kernel_size=[2, 2, 2])
                    x = tf.compat.v1.layers.batch_normalization(x, training=is_training, renorm=False) \
                        if self.batch_normalization else x
                    x = self.activation_fn(x)
                    x = tf.nn.dropout(x, 1 - (keep_prob))
