...
def input_fn(file_references, mode,
    ...
    dataset = dataset.map(lambda filename: tuple(tf.compat.v1.py_func(get_anotations_labels_decode, [filename, reports_path],
...
def model_fn(features, labels, mode, params):
    def specific_block(feats):
        ...
        for n_units in specific_units:
            feats = tf.compat.v1.layers.dense(feats, n_units, tf.nn.selu)
    ...
    for l in range(hidden_layers):
        x = tf.compat.v1.layers.dense(x, hidden_units, activation=tf.nn.selu)
        x = dropout_selu(x, dropout_rate)

    logits = [tf.compat.v1.layers.dense(specific_block(x), units=1) for task in range(7)]
    ...
    loss = tf.compat.v1.losses.sigmoid_cross_entropy(y, logits)
    ...
    AUC_CTR_left_lung_affected = tf.compat.v1.metrics.auc(y[:, 0], probabilities[:, 0])
    AUC_CTR_right_lung_affected = tf.compat.v1.metrics.auc(y[:, 1], probabilities[:, 1])
    AUC_CTR_lung_capacity_decrease = tf.compat.v1.metrics.auc(y[:, 2], probabilities[:, 2])
    AUC_CTR_calcification = tf.compat.v1.metrics.auc(y[:, 3], probabilities[:, 3])
    AUC_CTR_pleurisy = tf.compat.v1.metrics.auc(y[:, 4], probabilities[:, 4])
    AUC_CTR_caverns = tf.compat.v1.metrics.auc(y[:, 5], probabilities[:, 5])
    AUC_SVR_severity = tf.compat.v1.metrics.auc(y[:, 6], probabilities[:, 6])
