...
def decode_2(serialized_example, shape=[180, 180, 165, 1]):
    ...
    features = tf.parse_single_example(
        serialized_example,
        features={'image_raw': tf.FixedLenFeature(shape, tf.float32),
                  'labels': tf.FixedLenFeature([7], tf.float32), 
...
def tfrecord_to_image_itk_image(tfrecords_file, imgs_to_return = 100):
    record_iterator = tf.python_io.tf_record_iterator(path=tfrecords_file)
