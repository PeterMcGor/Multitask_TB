...
def convolution(x, filter, padding='SAME', strides=None, dilation_rate=None):
    ...
    tf.cond(tf.equal(tf.keras.backend.any(tf.is_nan(w)), False), lambda: tf.summary.histogram(scope_name+'/weights', w),
            lambda: tf.as_string(w))
    tf.cond(tf.equal(tf.keras.backend.any(tf.is_nan(b)), False), lambda: tf.summary.histogram(scope_name + '/biases', b),
            lambda: tf.as_string(b))
...
def deconvolution(x, filter, output_shape, strides, padding='SAME'):
    ...
    scope_name = tf.get_default_graph().get_name_scope()
    tf.summary.histogram(scope_name + '/weights', w)
    tf.summary.histogram(scope_name + '/biases', b)
