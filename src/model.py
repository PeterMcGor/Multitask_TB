...
def multi_task_block(x, meta_data, n_channels, uncertainty_type=UNCERTANTY_TYPE.HOMOCEDASTIC,
    ...
    def dense_block(input_layer, name, layers_units, activations, bias_initializer_last_layer):
        ...
        for i, layer_i in enumerate(layers_units[:-1]):
            output_layer = tf.layers.dense(output_layer, units=layer_i, name=name + '_' + str(layer_i),
                                           kernel_initializer=tf.keras.initializers.random_normal(stddev=0.01))
            ### TODO Este tiene pinta de ser 0 por como funciona #########
            output_layer = tf.layers.batch_normalization(output_layer, training=training, renorm=renorm) \
                if batch_normalization else output_layer
            print("GRAPH_NAME_2", tf.get_default_graph().get_name_scope())
            tf.reduce_mean(output_layer, name='MEAN_OUT_LAYER')
            #  tf.reduce_mean()
            tf.reduce_min(output_layer, name='MIN_OUT_LAYER')
            tf.reduce_max(output_layer, name='MAX_OUT_LAYER')
            ...
            if len(output_layer_vars) > 1:
                tf.summary.histogram(name + '_' + str(layer_i) + "_weights", output_layer_vars[0])
                tf.summary.histogram(name + '_' + str(layer_i) + "_biases", output_layer_vars[1])
                tf.summary.histogram(name + '_' + str(layer_i) + "_activations", output_layer)

        preds = tf.layers.dense(output_layer, units=layers_units[-1], name=name + '_' + str(layers_units[-1]),
                                activation=activations[-1], kernel_initializer=tf.initializers.zeros(),
                                bias_initializer=tf.keras.initializers.constant(bias_initializer_last_layer))

        output_layer_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=tf.get_default_graph().
                                              get_name_scope() + '/' + name + '_' + str(layers_units[-1]))
        if len(output_layer_vars) > 1:
            tf.summary.histogram(name + '_' + str(layers_units[-1]) + "_weights", output_layer_vars[0])
            tf.summary.histogram(name + '_' + str(layers_units[-1]) + "_biases", output_layer_vars[1])
            tf.summary.histogram(name + '_' + str(layers_units[-1]) + "_activations", output_layer)

        return output_layer, preds

    def get_log_var_uncertainty(name, uncertainty, input_layer=None):
        log_var = 0.
        with tf.variable_scope("log_variances"):
            if uncertainty == UNCERTANTY_TYPE.HOMOCEDASTIC:
                suffix = '_log_var_hom'
                log_var = tf.get_variable(name + suffix, shape=(1,), initializer=tf.contrib.layers.xavier_initializer(), # TODO ojo este Xavier --> RELU and 2 factor
    ...
    for manifestation in manifestations_list:
        with tf.variable_scope(manifestation + "_features"):
            ...
            tf.summary.scalar('log_var_' + unc_id + '_' + manifestation, tf.reduce_mean(manifestation_log_var))
    ...
    def __get_model__(self):
        if self.architecture == self.VNet:
            return VNet(1, keep_prob=self.arq_params['keep_prob'],
                        ...
                        batch_normalization=self.arq_params['batch_normalization'],
                        renormalization=self.arq_params['renormalization'],
                        activation_after_add=self.arq_params['activation_after_add'])
        elif self.architecture == self.UNet:
            return UNet(1, n_base_filters=16,
                        depth=self.arq_params['num_levels'],
                        batch_norm=self.arq_params['batch_normalization'])
        else:
            print("Incorrect architecture", self.architecture)
            sys.exit(1)
    ...
    def model_fn(self, features, labels, mode, params):
        ...
        def get_segmentation_summaries():
            for batch in range(1):
                ...
                tf.summary.image("image", tf.transpose(features_predictions_log, [2, 1, 0, 3]), max_outputs=max_outputs)
                tf.summary.image("uncertainty_seg", tf.transpose(log_var_log, [2, 1, 0, 3]), max_outputs=max_outputs)
                tf.summary.image("probs_seg", tf.transpose(seg_predictions_log, [2, 1, 0, 3]), max_outputs=max_outputs)
                tf.summary.image("class_seg", tf.transpose(seg_classes_log, [2, 1, 0, 3]), max_outputs=max_outputs)
                tf.summary.image("masks", tf.transpose(mask_log, [2, 1, 0, 3]), max_outputs=max_outputs)
        ...
        if self.aim != Aim.SEGMENTATION:  # Get classification
            with tf.variable_scope('after_' + self.architecture + '_features'):
            ...
            with tf.variable_scope("multi_task_features"):
        ...
        if self.aim != Aim.CLASSIFICATION:
            ...
            eval_metric_ops_seg = {"accuracy": tf.metrics.accuracy(labels=mask, predictions=seg_predictions["classes"]),
                                   "IOU": tf.metrics.mean_iou(labels=mask, predictions=seg_predictions["classes"],
                                                              num_classes=2)}
        ...
        if self.aim != Aim.SEGMENTATION:
            ...
            for manifestation in IMAGE_CLEF_KEYS.keys_as_list(just_binary=True):
                ...
                max = tf.clip_by_value(tf.reduce_max(softmax, axis=1,
                                                     name='Max_' + manifestation), tf.keras.backend.epsilon(),
                                       1 - tf.keras.backend.epsilon())
                arg_max = tf.cast(tf.argmax(softmax, axis=1, name='argmax_' + manifestation), max.dtype)
                ...
                max_2 = tf.clip_by_value(tf.reduce_max(softmax_2, axis=1, name='Max_2' + manifestation),
                                         tf.keras.backend.epsilon(), 1 - tf.keras.backend.epsilon())
                arg_max_2 = tf.cast(tf.argmax(softmax_2, axis=1, name='argmax_2' + manifestation), max_2.dtype)
            ...
            AUC_CTR_left_lung_affected = tf.metrics.auc(labels[IMAGE_CLEF_KEYS.CTR_LEFT_LUNG_AFFECTED],
                                                        binary_sigmoid[IMAGE_CLEF_KEYS.CTR_LEFT_LUNG_AFFECTED])
            AUC_CTR_right_lung_affected = tf.metrics.auc(labels[IMAGE_CLEF_KEYS.CTR_RIGHT_LUNG_AFFECTED],
                                                         binary_sigmoid[IMAGE_CLEF_KEYS.CTR_RIGHT_LUNG_AFFECTED])
            AUC_CTR_lung_capacity_decrease = tf.metrics.auc(labels[IMAGE_CLEF_KEYS.CTR_LUNG_CAPACITY_DECREASE],
                                                            binary_sigmoid[IMAGE_CLEF_KEYS.CTR_LUNG_CAPACITY_DECREASE])
            AUC_CTR_calcification = tf.metrics.auc(labels[IMAGE_CLEF_KEYS.CTR_CALCIFICATION],
                                                   binary_sigmoid[IMAGE_CLEF_KEYS.CTR_CALCIFICATION])
            AUC_CTR_pleurisy = tf.metrics.auc(labels[IMAGE_CLEF_KEYS.CTR_PLEURISY],
                                              binary_sigmoid[IMAGE_CLEF_KEYS.CTR_PLEURISY])
            AUC_CTR_caverns = tf.metrics.auc(labels[IMAGE_CLEF_KEYS.CTR_CAVERNS],
            ...
            AUC_SVR_severity = tf.metrics.auc(labels[IMAGE_CLEF_KEYS.SVR_SEVERITY], severity_sigmoid)

            AUC_CTR_left_lung_affected_2 = tf.metrics.auc(labels[IMAGE_CLEF_KEYS.CTR_LEFT_LUNG_AFFECTED],
                                                          binary_sigmoid_2[IMAGE_CLEF_KEYS.CTR_LEFT_LUNG_AFFECTED])
            AUC_CTR_right_lung_affected_2 = tf.metrics.auc(labels[IMAGE_CLEF_KEYS.CTR_RIGHT_LUNG_AFFECTED],
                                                           binary_sigmoid_2[IMAGE_CLEF_KEYS.CTR_RIGHT_LUNG_AFFECTED])
            AUC_CTR_lung_capacity_decrease_2 = tf.metrics.auc(labels[IMAGE_CLEF_KEYS.CTR_LUNG_CAPACITY_DECREASE],
                                                              binary_sigmoid_2[
                                                                  IMAGE_CLEF_KEYS.CTR_LUNG_CAPACITY_DECREASE])
            AUC_CTR_calcification_2 = tf.metrics.auc(labels[IMAGE_CLEF_KEYS.CTR_CALCIFICATION],
                                                     binary_sigmoid_2[IMAGE_CLEF_KEYS.CTR_CALCIFICATION])
            AUC_CTR_pleurisy_2 = tf.metrics.auc(labels[IMAGE_CLEF_KEYS.CTR_PLEURISY],
                                                binary_sigmoid_2[IMAGE_CLEF_KEYS.CTR_PLEURISY])
            AUC_CTR_caverns_2 = tf.metrics.auc(labels[IMAGE_CLEF_KEYS.CTR_CAVERNS],
                                               binary_sigmoid_2[IMAGE_CLEF_KEYS.CTR_CAVERNS])
            AUC_SVR_severity_2 = tf.metrics.auc(labels[IMAGE_CLEF_KEYS.SVR_SEVERITY], severity_sigmoid_2)

            eval_metric_ops_clf = {
                "RMSE_" + IMAGE_CLEF_KEYS.SEVERITY: tf.metrics.root_mean_squared_error(
                ...
                "ACC_" + IMAGE_CLEF_KEYS.SVR_SEVERITY: tf.metrics.accuracy(labels[IMAGE_CLEF_KEYS.SVR_SEVERITY],
                                                                           severity_sigmoid > 0.5),

                "ACC_" + IMAGE_CLEF_KEYS.CTR_LEFT_LUNG_AFFECTED: tf.metrics.accuracy(
                    labels[IMAGE_CLEF_KEYS.CTR_LEFT_LUNG_AFFECTED],
                    prediction[IMAGE_CLEF_KEYS.CTR_LEFT_LUNG_AFFECTED]),

                "ACC_" + IMAGE_CLEF_KEYS.CTR_RIGHT_LUNG_AFFECTED: tf.metrics.accuracy(
                    labels[IMAGE_CLEF_KEYS.CTR_RIGHT_LUNG_AFFECTED],
                    prediction[IMAGE_CLEF_KEYS.CTR_RIGHT_LUNG_AFFECTED]),

                "ACC_" + IMAGE_CLEF_KEYS.CTR_LUNG_CAPACITY_DECREASE: tf.metrics.accuracy(
                    labels[IMAGE_CLEF_KEYS.CTR_LUNG_CAPACITY_DECREASE],
                    prediction[IMAGE_CLEF_KEYS.CTR_LUNG_CAPACITY_DECREASE]),

                "ACC_" + IMAGE_CLEF_KEYS.CTR_CALCIFICATION: tf.metrics.accuracy(
                    labels[IMAGE_CLEF_KEYS.CTR_CALCIFICATION],
                    prediction[IMAGE_CLEF_KEYS.CTR_CALCIFICATION]),

                "ACC_" + IMAGE_CLEF_KEYS.CTR_PLEURISY: tf.metrics.accuracy(
                    labels[IMAGE_CLEF_KEYS.CTR_PLEURISY],
                    prediction[IMAGE_CLEF_KEYS.CTR_PLEURISY]),

                "ACC_" + IMAGE_CLEF_KEYS.CTR_CAVERNS: tf.metrics.accuracy(
                    labels[IMAGE_CLEF_KEYS.CTR_CAVERNS],
                    prediction[IMAGE_CLEF_KEYS.CTR_CAVERNS]),

                "FPE_" + IMAGE_CLEF_KEYS.CTR_LEFT_LUNG_AFFECTED: tf.metrics.false_positives(
                    labels[IMAGE_CLEF_KEYS.CTR_LEFT_LUNG_AFFECTED],
                    prediction[IMAGE_CLEF_KEYS.CTR_LEFT_LUNG_AFFECTED]),

                "FPE_" + IMAGE_CLEF_KEYS.CTR_RIGHT_LUNG_AFFECTED: tf.metrics.false_positives(
                    labels[IMAGE_CLEF_KEYS.CTR_RIGHT_LUNG_AFFECTED],
                    prediction[IMAGE_CLEF_KEYS.CTR_RIGHT_LUNG_AFFECTED]),

                "FPE_" + IMAGE_CLEF_KEYS.CTR_LUNG_CAPACITY_DECREASE: tf.metrics.false_positives(
                    labels[IMAGE_CLEF_KEYS.CTR_LUNG_CAPACITY_DECREASE],
                    prediction[IMAGE_CLEF_KEYS.CTR_LUNG_CAPACITY_DECREASE]),

                "FPE_" + IMAGE_CLEF_KEYS.CTR_CALCIFICATION: tf.metrics.false_positives(
                    labels[IMAGE_CLEF_KEYS.CTR_CALCIFICATION],
                    prediction[IMAGE_CLEF_KEYS.CTR_CALCIFICATION]),

                "FPE_" + IMAGE_CLEF_KEYS.CTR_PLEURISY: tf.metrics.false_positives(
                    labels[IMAGE_CLEF_KEYS.CTR_PLEURISY],
                    prediction[IMAGE_CLEF_KEYS.CTR_PLEURISY]),

                "FPE_" + IMAGE_CLEF_KEYS.CTR_CAVERNS: tf.metrics.false_positives(
                    labels[IMAGE_CLEF_KEYS.CTR_CAVERNS],
                    prediction[IMAGE_CLEF_KEYS.CTR_CAVERNS]),

                "FNE_" + IMAGE_CLEF_KEYS.CTR_LEFT_LUNG_AFFECTED: tf.metrics.false_negatives(
                    labels[IMAGE_CLEF_KEYS.CTR_LEFT_LUNG_AFFECTED],
                    prediction[IMAGE_CLEF_KEYS.CTR_LEFT_LUNG_AFFECTED]),

                "FNE_" + IMAGE_CLEF_KEYS.CTR_RIGHT_LUNG_AFFECTED: tf.metrics.false_negatives(
                    labels[IMAGE_CLEF_KEYS.CTR_RIGHT_LUNG_AFFECTED],
                    prediction[IMAGE_CLEF_KEYS.CTR_RIGHT_LUNG_AFFECTED]),

                "FNE_" + IMAGE_CLEF_KEYS.CTR_LUNG_CAPACITY_DECREASE: tf.metrics.false_negatives(
                    labels[IMAGE_CLEF_KEYS.CTR_LUNG_CAPACITY_DECREASE],
                    prediction[IMAGE_CLEF_KEYS.CTR_LUNG_CAPACITY_DECREASE]),

                "FNE_" + IMAGE_CLEF_KEYS.CTR_CALCIFICATION: tf.metrics.false_negatives(
                    labels[IMAGE_CLEF_KEYS.CTR_CALCIFICATION],
                    prediction[IMAGE_CLEF_KEYS.CTR_CALCIFICATION]),

                "FNE_" + IMAGE_CLEF_KEYS.CTR_PLEURISY: tf.metrics.false_negatives(
                    labels[IMAGE_CLEF_KEYS.CTR_PLEURISY],
                    prediction[IMAGE_CLEF_KEYS.CTR_PLEURISY]),

                "FNE_" + IMAGE_CLEF_KEYS.CTR_CAVERNS: tf.metrics.false_negatives(
                    labels[IMAGE_CLEF_KEYS.CTR_CAVERNS],
                    prediction[IMAGE_CLEF_KEYS.CTR_CAVERNS]),

                ##metrics.mean just do the trick in order to show the eval loss per task
                "LOSS_EVAL_" + IMAGE_CLEF_KEYS.SEVERITY: tf.metrics.mean(av_per_task_loss[IMAGE_CLEF_KEYS.SEVERITY]),
                "LOSS_EVAL_" + IMAGE_CLEF_KEYS.CTR_LEFT_LUNG_AFFECTED: tf.metrics.mean(
                    av_per_task_loss[IMAGE_CLEF_KEYS.CTR_LEFT_LUNG_AFFECTED]),
                "LOSS_EVAL_" + IMAGE_CLEF_KEYS.CTR_RIGHT_LUNG_AFFECTED: tf.metrics.mean(
                    av_per_task_loss[IMAGE_CLEF_KEYS.CTR_RIGHT_LUNG_AFFECTED]),
                "LOSS_EVAL_" + IMAGE_CLEF_KEYS.CTR_LUNG_CAPACITY_DECREASE: tf.metrics.mean(
                    av_per_task_loss[IMAGE_CLEF_KEYS.CTR_LUNG_CAPACITY_DECREASE]),
                "LOSS_EVAL_" + IMAGE_CLEF_KEYS.CTR_CALCIFICATION: tf.metrics.mean(
                    av_per_task_loss[IMAGE_CLEF_KEYS.CTR_CALCIFICATION]),
                "LOSS_EVAL_" + IMAGE_CLEF_KEYS.CTR_PLEURISY: tf.metrics.mean(
                    av_per_task_loss[IMAGE_CLEF_KEYS.CTR_PLEURISY]),
                "LOSS_EVAL_" + IMAGE_CLEF_KEYS.CTR_CAVERNS: tf.metrics.mean(
        ...
        if mode == tf.estimator.ModeKeys.EVAL:
            ...
            if self.aim == Aim.CLASS_AND_SEG:
                eval_metric_ops_seg = {**eval_metric_ops_seg,
                                       **{"LOSS_SEG": tf.metrics.mean(av_per_task_loss['Loss_seg'])}}
