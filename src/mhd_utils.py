...
def read_meta_header(filename):
    ...
    fileIN = file_io.FileIO(filename, mode = 'r')
...
def load_raw_data_with_mhd(filename):
    """
    Opens images from the filesystem or in a google bucket(gs://)
    through a not smart workaround
    """
    ...
    data = file_io.FileIO(data_file,mode='rb').read()
    data = np.fromstring(data,count=np.prod(shape),dtype = np_type)
    #with file_io.FileIO(data_file, mode = 'rb') as fid:
    #with open(data_file,'rb') as fid:
        #data = np.fromfile(fid, count=np.prod(shape),dtype = np_type)
        #data=np.frombuffer(fid, count=np.prod(shape),dtype = np_type)
