...
LOSS_FN_DICT = {'RMSE': tf.losses.mean_squared_error,
...
OPTIMIZER_DICT = {'SGD': tf.train.GradientDescentOptimizer,
...
def main(argv):
    ...
    # Config hardware strategy. Mirrors and estimators are the easiest way for portable code between GCP and
    # local-machines
    ...
    # Vnet_decoder needs te instantiated model in order to extract layers
    ...
    reader_examples_types = {'features':
                                 {'x': tf.float32,
                                  'meta_info': tf.string if classification_task == Aim.SEGMENTATION else tf.float32},
                             'labels': {'Mask': tf.string if classification_task == Aim.CLASSIFICATION else tf.float32,
                                        'y': tf.string if classification_task == Aim.SEGMENTATION else tf.float32,
                                        'Name': tf.string}}
    #
    ...
    tensors_to_log = {'Check_rand': 'Check_rand',
                      'MODE_NAME': 'MODE_NAME'}  # 'MEAN_OUT_LAYER':'MEAN_OUT_LAYER',  'MIN_OUT_LAYER': 'MIN_OUT_LAYER', 'MAX_OUT_LAYER':'MAX_OUT_LAYER''CE_per_sample': 'CE_per_sample'}
    levels = [3, 4]
    covs = [1, 2, 3]
    ...
    encoder_hooks = [{'cnn_features/vnet/encoder/level_' + str(l) + '/conv_' + str(
        c) + '/' + KPI: 'cnn_features/vnet/encoder/level_' + str(l) + '/conv_' + str(c) + '/' + KPI} for l in levels for
                     c in covs for KPI in KPIS]
    encoder_hooks_dct = {}
    for dct in encoder_hooks:
        encoder_hooks_dct = {**encoder_hooks_dct, **dct}
    dense_hooks = [{
        'multi_task_features/' + manifestation + '_features/' + KPI2: 'multi_task_features/' + manifestation + '_features/' + KPI2
        for manifestation in IMAGE_CLEF_KEYS.keys_as_list() for KPI2 in KPIS_2}]
    dense_hooks_dct = {}
    ...
    logging_hook_2 = tf.train.LoggingTensorHook(tensors=tensors_to_log, every_n_iter=1)
    eval_spec = tf.estimator.EvalSpec(input_fn_reader_val, name='validation',
                                      ...
                                      exporters=[exporter])  # hooks=[logging_hook_2])
